...
async function signup(event) {
  ...
  if(!hasSpecialChars(password) || !hasUppercase(password) || !hasLowercase(password) || !hasNumber(password)){
    showErrorText("Please ensure your password has at least one lowercase letter, capital letter, number and special character.");
    hideLoadingScreen();
    return;
  }

  if(!testMinLength(password, 10)){
    showErrorText("Please ensure your password is at least 10 characters in length.");
    hideLoadingScreen();
    return;
  }

  if(!testMaxLength(password, 255) || !testMaxLength(username, 255)){
    showErrorText("Please ensure your username and password are less than 255 characters in length.");
    hideLoadingScreen();
    return;
  }
